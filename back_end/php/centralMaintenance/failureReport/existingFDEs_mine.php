		...
		for ($j = 0; $j < count($res[$i]->FDE); $j++) {
			...
			if (mysqli_num_rows($result_fde) > 0) {
				while ($row_fde = mysqli_fetch_assoc($result_fde)) {
					...
					array_push($res[$i]->FDEStatus, $row_fde["fde_state"]);
	...
	for ($j = 0; $j < count($res[$i]->FDE); $j++) {
		...
		if ($FDEIndex > 0) {
			...
			if ($FDEFlag == 0) {
				...
				$itemFDE->FDEText = $res[$i]->FDEText;
				$itemFDE->FDEStatus = $res[$i]->FDEStatus;
				$itemFDE->FDEClass = $res[$i]->FDEClass;
