...
export default {
  ...
  methods: {
    ...
    handleTestRowClick(row) {
      ...
      if (this.selectedTests.includes(row)) {
      ...
      } else if (this.$store.state.groundTestList.InterferingTestIndexs.includes(row.InitiatedTest_Index)) {
        this.$message({
          type: 'warning',
          message: 'This text is interfering!'
        });
      } else if (['1', '3', '4'].includes(row.InitiatedTest_Status)) {
        this.$message({
          type: 'warning',
        ...
        this.$confirm('Do you want to add parameter ', row.InitiatedTestName, ' to the list to be opened?', 'Tips', {
          ...
          this.$store.state.groundTestList.InterferingTestIndexs = [...this.$store.state.groundTestList.InterferingTestIndexs, ...row.InterferingTests_Index.split(';').filter(Boolean)];
          console.log(this.$store.state.groundTestList.InterferingTestIndexs)
